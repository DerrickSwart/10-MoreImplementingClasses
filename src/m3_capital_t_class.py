    ...
    *** See the PDFs, especially dimensions.pdf, to help you understand this.
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        *** See   dimensions.pdf   to understand the following! ***
        ...
        # ---------------------------------------------------------------------
        h_rect_x_1 = intersection_center.x - (width/2)
        h_rect_y_1 = intersection_center.y - (letter_thickness/2)
        h_rect_x_2 = intersection_center.x + (width / 2)
        h_rect_y_2 = intersection_center.y + (letter_thickness / 2)

        v_rect_x_1 = intersection_center.x - (letter_thickness/2)
        v_rect_y_1 = intersection_center.y - (letter_thickness/2)
        v_rect_x_2 = intersection_center.x + (letter_thickness/2)
        v_rect_y_2 = intersection_center.y + (height - (letter_thickness/2))

        self.h_rect = rg.Rectangle(rg.Point(h_rect_x_1, h_rect_y_1), rg.Point(h_rect_x_2, h_rect_y_2))
        self.v_rect = rg.Rectangle(rg.Point(v_rect_x_1, v_rect_y_1), rg.Point(v_rect_x_2, v_rect_y_2))
    ...
    def clone(self):
        ...
        #######################################################################
        point = self.h_rect.get_center()
        width = self.h_rect.get_width()
        height = self.v_rect.get_height()
        thick = self.h_rect.get_height()
        new_t = CapitalT(point,width,height, thick)
